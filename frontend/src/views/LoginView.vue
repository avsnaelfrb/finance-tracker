...
const handleLogin = async () => {
  ...
  if (!formData.email || !formData.password) {
    Swal.fire({
      icon: 'warning',
      title: 'Input Kosong',
      text: 'Mohon isi email dan password Anda.',
  ...
  try {
    ...
    errMsg.value = err.response?.data?.message || err.message || 'Terjadi kesalahan saat mencoba masuk.';
  } finally {
...
<template>
  <div class="card-base group">
    <!-- Header Section -->
    <!-- HEADER SECTION: Dynamic Switching -->
    <div class="mb-6 min-h-[4rem] flex flex-col justify-center">
      ...
      <div v-if="errMsg" class="animate-fade-in p-4 rounded-xl bg-red-50 border border-red-100 dark:bg-red-900/20 dark:border-red-800 flex items-start gap-3">
        ...
        <div class="flex-1">
          <h3 class="text-sm font-bold text-red-800 dark:text-red-300">Gagal Masuk</h3>
      ...
      <div v-else class="animate-fade-in">
        <h2 class="text-xl font-bold text-gray-900 dark:text-white font-sans">Selamat Datang Kembali</h2>
    ...
    <form @submit.prevent="handleLogin" class="space-y-5">
      ...
      <div class="space-y-1.5">
        ...
        <div class="input-group">
          ...
          <input 
            ...
            required 
          />
          <!-- Tombol Toggle Show Password (Fitur yang dipertahankan) -->
          <button 
            type="button" 
      ...
      <button type="submit" class="btn-primary group mt-2" :disabled="isLoading">
        ...
        <span v-else class="flex items-center gap-2">
          Masuk <ArrowRight class="h-4 w-4 transition-transform group-hover:translate-x-1" />
        </span>
      </button>
    </form>
    ...
    <div class="relative my-6">
      ...
      <div class="relative flex justify-center text-xs uppercase">
        <span class="bg-white px-2 text-gray-500 dark:bg-dark-800 dark:text-gray-400">BELUM PUNYA AKUN?</span>
      </div>
    </div>
    ...
    <div class="text-center">
      <p class="text-sm text-gray-600 dark:text-gray-400">
        <router-link to="/register" class="font-bold text-brand-600 hover:text-brand-500 hover:underline dark:text-brand-400">
          Buat akun baru
        </router-link>
      </p>
    </div>
